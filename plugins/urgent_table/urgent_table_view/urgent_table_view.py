...
class UrgentTableView:
    def __init__(self):
        self.layout_settings = {}
    def get_plugin_layout_settings(self):
        # print("Current dir: " + os.getcwd())
        with open('plugins/urgent_table/urgent_table_config/urgent_table_config.json') as json_plugin_config:
    ...
    def create_plugin_switch(self):
        return daq.BooleanSwitch(
            id='urgent-table-switch',
            label={"label": "Urgent Dropped Table",
    ...
    def create_plugin_layout(self):
        return dbc.Card(
            dbc.Row([
                dbc.Col(dbc.Card(id='display-urgent-table'), width=base_display_const.MAX_VISUALIZATION_WIDTH)
            ]),
            id="urgent-table-cover")

    def draw_urgent_table(self, input_data):
        # data_input_for_a_route = processing_data(self, route_index)
        return [html.H2("Urgent Dropped Request Table"),
