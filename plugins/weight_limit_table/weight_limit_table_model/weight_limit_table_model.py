...
class WeightLimitTableModel:
    ...
    def processing_data_to_view(self, routes_data):
        ...
        for route_idx, route in enumerate(routes_data):
            ...
            weight_limit_table_input["data"].append(row_data)
            weight_limit_table_input["cell_colors"].append(row_colors)

        # Now fishbone_table_input is prepared with the data and cell_colors according to the requirements
        weight_limit_table_input["cell_colors"] = list(
            map(list, zip(*weight_limit_table_input["cell_colors"])))  # Transpose the cell colors

        return weight_limit_table_input
